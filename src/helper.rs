...
// fetching the index
pub fn index(range: u64, rndm: f64) -> u64 {
    let idx = (range - 1) as f64 * rndm;
    idx as u64
}

// main function 
pub fn generate_random_number(client_seed: &str, nonce: u8, range_start: u64, range_end: u64) -> u64 {
    ...
    // this number here, 23.22 is just a random number, experimenting and tweaking this number will give much crispier results when there are bigger numbers
    frnd = (frnd * 10.0) / 23.22;

    let idx = index(range_end, 
    +);
    msg!(&format!("frnd: {}, idx: {}, start: {}, end: {}", frnd, idx, range_start, range_end));
    let random_number = idx % (range_end - range_start + 1) + range_start;
    return random_number as u64;   
}